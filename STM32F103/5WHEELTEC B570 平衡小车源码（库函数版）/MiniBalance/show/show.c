...
extern int Encoder_Left,Encoder_Right,dusu,Position_Left;   
/**************************************************************************
...
{
		//=============第一行显示小车模式=======================//	
		      OLED_ShowString(00,0,"dusu");
						if(dusu<0)			OLED_ShowString(48,0,"-");
						if(dusu>=0)		OLED_ShowString(48,0,"+");
		
													OLED_ShowNumber(56,0,myabs((int)dusu),5,12);
		//=============第二行显示角度=======================//	
	  ...
	  //=============第三行显示角速度与距离===============//	
													OLED_ShowString(0,20,"Gr_Z");
		                 if(Gyro_Turn<0)	  OLED_ShowString(45,20,"-");
		                 if(Gyro_Turn>=0)	  OLED_ShowString(45,20,"+");
	                 
		                      OLED_ShowNumber(55,20, myabs((int)Gyro_Turn),4,12);
		...
		if(Position_Left<0)		  OLED_ShowString(16,30,"-"),
													OLED_ShowNumber(26,30,myabs((int)Position_Left),4,12);
		if(Position_Left>=0)	    OLED_ShowString(16,30,"+"),
		                      OLED_ShowNumber(26,30,myabs((int)Position_Left),4,12);
													
		if(Encoder_Left<0)	  OLED_ShowString(60,30,"-");
		if(Encoder_Left>=0)	OLED_ShowString(60,30,"+");
		                      OLED_ShowNumber(68,30,myabs((int)Encoder_Left),4,12);
													OLED_ShowString(96,30,"  ");
	
		//=============第五行显示右编码器PWM与读数=======================//		
		                      OLED_ShowString(00,40,"R");
		if(Motor_Right<0)		  OLED_ShowString(16,40,"-"),
													OLED_ShowNumber(26,40,myabs((int)Encoder_Right),4,12);
		if(Motor_Right>=0)	  OLED_ShowString(16,40,"+"),
		                      OLED_ShowNumber(26,40,myabs((int)Encoder_Right),4,12);
													
		if(Encoder_Right<0)	OLED_ShowString(60,40,"-");
		if(Encoder_Right>=0)	OLED_ShowString(60,40,"+");
		                      OLED_ShowNumber(68,40,myabs((int)Encoder_Right),4,12);
													OLED_ShowString(96,40,"  ");

		//=============第六行显示电压与电机开关=======================//
//		                      OLED_ShowString(0,50,"dusu");
//						if(dusu<0)			OLED_ShowString(42,50,"-");
//						if(dusu>=0)		OLED_ShowString(42,50,"+");
//		
//													OLED_ShowNumber(47,50,myabs((int)dusu),5,12);
//													//OLED_ShowNumber(42,50, ,5,12);
		
		
		 OLED_ShowString(0,50,"Yaw");
						if(Yaw<0)			OLED_ShowString(42,50,"-");
						if(Yaw>=0)		OLED_ShowString(42,50,"+");
		
													OLED_ShowNumber(47,50,myabs((int)Yaw),5,12);
		
		
		
		if(!Flag_Stop)         OLED_ShowString(89,50,"OFF");
		if(Flag_Stop)        OLED_ShowString(89,50,"ON ");
